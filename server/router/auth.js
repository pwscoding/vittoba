...
router.post('/add_product', async (req, res) => {
  ...
  try {
    const { product_name, category, color, description, material, price, quantity, size, weight, cta, makeitfeatured } = req.body.values
    const newProduct = new ProductSchema({ product_name, category, color, description, material, price, quantity, size, weight, cta, images0: [], images1: [], images2: [], images3: [], images4: [], images5: [], images6: [], makeitfeatured })
...
router.post('/update_product', async (req, res) => {
  ...
  try {
    ...
    ProductSchema.findByIdAndUpdate(selectedKey, { product_name, category, color, description, material, price, quantity, size, weight, cta, makeitfeatured }, (error) => {
      if (error) {
        console.log(error);
      } else {
